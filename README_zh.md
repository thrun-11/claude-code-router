...
Claude Code Router Desktop 是给编程 Agent 用的本地控制平面。它为 Claude Code、Codex、ZCode 以及兼容 API 客户端提供一个稳定的本地入口，然后由你在 CCR 中决定每个请求应该走哪个供应商、哪个模型、哪套路由策略、哪些工具能力和哪组账号凭据。
...
| 目标 | CCR 提供的能力 |
| --- | --- |
| 保持 Agent 工作流不变，同时自由切换模型 | 为 Claude Code、Codex、ZCode 创建本地配置档案，支持 CLI / App 启动入口和按配置选择模型 |
...
- **Agent 配置档案**：为 Claude Code、Codex 和 ZCode 创建配置档案，支持模型覆盖、作用范围、CLI / App 启动方式、环境变量和多开 App 工作流。
...
打开 **Agent配置**，选择要使用的客户端。配置 Claude Code、Codex 或 ZCode，选择目标模型和作用范围，然后应用配置。对于 App 入口，可以使用 **打开 Agent** 通过 CCR 打开目标应用。
