...
var defaultVisionBaseUrl = "https://api.openai.com/v1";
var defaultVisionModel = "gpt-4o-mini";
var defaultTimeoutMs = 3e4;
var maxLocalImageBytes = 20 * 1024 * 1024;
var fusionUsageEventSchema = "ccr.fusion-usage.v1";
var fusionUsageSyncBaseDelayMs = 100;
var fusionUsageSyncDrainTimeoutMs = 15e3;
var fusionUsageSyncMaxAttempts = 3;
var fusionUsageSyncTimeoutMs = 5e3;
var pendingUsageSyncs = /* @__PURE__ */ new Set();
var usageSyncFailureLogged = false;
var usageSyncShutdownStarted = false;
var toolKind = parseToolKind(env("FUSION_BUILTIN_TOOL_KIND"));
var toolName = env("FUSION_TOOL_NAME") || env("FUSION_VISION_TOOL_NAME") || (toolKind === "web_search" ? "web_search" : "vision_understand");
var toolTitle = env("FUSION_TOOL_TITLE") || env("FUSION_VISION_TOOL_TITLE") || (toolKind === "web_search" ? "Fusion Web Search" : "Fusion Vision Understand");
var visionTool = {
...
var activeTool = toolKind === "web_search" ? webSearchTool : visionTool;
  ...
  try {
    switch (request.method) {
      ...
      case "ping":
        return jsonRpcResult(id, {});
      case "tools/list":
        return jsonRpcResult(id, { tools: [activeTool] });
      case "tools/call":
...
async function analyzeVision(args) {
  ...
  const timeoutMs = clampInteger(readNumber(args.timeoutMs) ?? readNumber(env("VISION_TIMEOUT_MS")) ?? defaultTimeoutMs, 100, 6e5);
...
function scheduleVisionUsageSync(input) {
  ...
  const target = input.gatewayRuntime ? splitGatewayVisionModelTarget(input.model) : { model: input.model };
...
async function publishVisionUsage(endpoint, header, token, event) {
  let lastError;
  for (let attempt = 1; attempt <= fusionUsageSyncMaxAttempts; attempt += 1) {
    try {
      ...
      const acknowledgement = rawBody ? parseJson(rawBody) : void 0;
...
var FusionUsageSyncHttpError = class extends Error {
...
var FusionUsageSyncRejectedError = class extends Error {
  retryable = false;
  constructor() {
...
function usageSyncErrorIsRetryable(error) {
  return !(error instanceof FusionUsageSyncRejectedError) && (!(error instanceof FusionUsageSyncHttpError) || error.retryable);
}
async function shutdownAfterUsageSyncDrain(exitCode) {
...
function readVisionUsage(response, payload) {
  ...
  const headerNumber = (name) => readNumber(response?.headers.get(name));
  return {
    cache_read_tokens: headerNumber("x-gateway-billing-cache-read-tokens") ?? readNumber(usage.cache_read_tokens) ?? readNumber(usage.cache_read_input_tokens) ?? readNumber(inputDetails.cached_tokens),
    cache_write_tokens: headerNumber("x-gateway-billing-cache-write-tokens") ?? readNumber(usage.cache_write_tokens) ?? readNumber(usage.cache_creation_input_tokens) ?? readNumber(inputDetails.cache_creation_tokens),
    input_tokens: headerNumber("x-gateway-billing-input-tokens") ?? readNumber(usage.input_tokens) ?? readNumber(usage.prompt_tokens),
    output_tokens: headerNumber("x-gateway-billing-output-tokens") ?? readNumber(usage.output_tokens) ?? readNumber(usage.completion_tokens),
    total_tokens: headerNumber("x-gateway-billing-total-tokens") ?? readNumber(usage.total_tokens)
  };
}
function readVisionCost(response, payload) {
  const headerCost = readNumber(response?.headers.get("x-gateway-billing-total-cost"));
  if (headerCost !== void 0) {
    return headerCost;
  }
  const root = isRecord(payload) ? payload : {};
...
function splitGatewayVisionModelTarget(model) {
  const separator = model.indexOf("/");
  const providerName = separator > 0 ? model.slice(0, separator).trim() : void 0;
  const physicalModel = separator > 0 && separator < model.length - 1 ? model.slice(separator + 1).trim() : model;
  const credentialId = providerName?.split("::").find((part) => part.startsWith("cred:"))?.slice("cred:".length).trim();
  return {
...
function usageOutcome(statusCode) {
...
async function analyzeWebSearch(args) {
  const prompt = readString(args.prompt);
  if (!prompt) {
    throw new Error(`${toolName} requires prompt.`);
  }
  ...
  const timeoutMs = clampInteger(readNumber(args.timeoutMs) ?? readNumber(env("SEARCH_TIMEOUT_MS")) ?? defaultTimeoutMs, 100, 6e5);
  const input = {
    count,
...
async function searchWithProvider(provider, input) {
...
async function buildImageParts(args, detail) {
  const inputs = [];
...
async function imageInputToUrl(input) {
  ...
  const buffer = await (0, import_promises.readFile)(input.path);
...
function extractResponseText(value) {
  ...
  const message = isRecord(first?.message) ? first.message : void 0;
  const content = message?.content;
  if (typeof content === "string") {
    return content;
  }
  if (Array.isArray(content)) {
    const text = content.map((item) => isRecord(item) ? readString(item.text) : void 0).filter((item) => Boolean(item)).join("\n");
    return text || void 0;
  }
  return readString(value.output_text);
}
function extractProviderError(rawText, json) {
  if (isRecord(json)) {
...
function resolveSearchProvider() {
  ...
  const candidates = ["brave", "bing", "google_cse", "serper", "serpapi", "tavily", "exa"];
...
function parseSearchProvider(value) {
  if (value === "auto" || value === "brave" || value === "bing" || value === "google_cse" || value === "serper" || value === "serpapi" || value === "tavily" || value === "exa") {
    return value;
  }
  return void 0;
}
function searchProviderIsConfigured(provider) {
...
function scopedSearchQuery(input) {
...
function freshnessToGoogleTbs(value) {
  if (value === "day") return "qdr:d";
  if (value === "week") return "qdr:w";
  if (value === "month") return "qdr:m";
  return void 0;
}
function normalizeSearchResult(value, titleKey, urlKey, snippetKey) {
  if (!isRecord(value)) {
    return {};
  }
  return {
...
function readStringArray(value) {
  ...
  return value.map(readString).filter((item) => Boolean(item));
}
...
function writeJsonRpc(response) {
  ...
  process.stdout.write(`Content-Length: ${Buffer.byteLength(payload, "utf8")}\r
\r
${payload}`);
}
