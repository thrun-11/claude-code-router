...
// packages/core/test/integration/agents/codex-cli-middleware-runtime.test.mjs
...
function codexCliMiddlewareRuntimeScript() {
...
// packages/core/test/integration/agents/codex-cli-middleware-runtime.test.mjs
(0, import_node_test.default)("generated Codex CLI middleware runtime is valid JavaScript", () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-check-"));
  const file = import_node_path.default.join(dir, "ccr-codex-cli-middleware.js");
  (0, import_node_fs.writeFileSync)(file, codexCliMiddlewareRuntimeScript());
  (0, import_node_child_process.execFileSync)(process.execPath, ["--check", file], { stdio: "pipe" });
});
(0, import_node_test.default)("Codex app-server uses ChatGPT's bundled Node as a signed supervisor", { skip: process.platform !== "darwin" }, () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-signed-supervisor-"));
  const runtimeFile = writeRuntimeScript(dir);
  const resourcesDir = import_node_path.default.join(dir, "ChatGPT.app", "Contents", "Resources");
  const fakeCodex = import_node_path.default.join(resourcesDir, "codex");
  const bundledNode = import_node_path.default.join(resourcesDir, "cua_node", "bin", "node");
  const observedFile = import_node_path.default.join(dir, "supervisor-args.txt");
  (0, import_node_fs.mkdirSync)(import_node_path.default.dirname(bundledNode), { recursive: true });
  (0, import_node_fs.writeFileSync)(fakeCodex, [
    "#!/bin/sh",
    "IFS= read -r request",
    `printf '%s\\n' '{"id":1,"result":{"supervised":true}}'`,
    ""
  ].join("\n"));
  (0, import_node_fs.writeFileSync)(bundledNode, [
    "#!/bin/sh",
    `printf '%s\\n' "$@" > "$CCR_FAKE_SUPERVISOR_ARGS"`,
    'exec "$CCR_TEST_NODE" "$@"',
    ""
  ].join("\n"));
  (0, import_node_fs.chmodSync)(fakeCodex, 448);
  (0, import_node_fs.chmodSync)(bundledNode, 448);
  const result = (0, import_node_child_process.spawnSync)(process.execPath, [runtimeFile, "app-server"], {
    encoding: "utf8",
    env: {
      ...process.env,
  ...
  import_strict.default.deepEqual(JSON.parse(result.stdout.trim()).result, { supervised: true });
  const observed = (0, import_node_fs.readFileSync)(observedFile, "utf8");
  import_strict.default.match(observed, /^-e\n/);
  import_strict.default.ok(observed.includes(fakeCodex));
  import_strict.default.ok(observed.includes("app-server"));
});
(0, import_node_test.default)("Codex runtime ignores middleware recursion and uses the bundled real CLI fallback", { skip: process.platform === "win32" }, () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-real-cli-fallback-"));
  const runtimeFile = writeRuntimeScript(dir);
  const fakeCodex = import_node_path.default.join(dir, "real-codex");
  (0, import_node_fs.writeFileSync)(fakeCodex, "#!/bin/sh\nprintf 'real-codex\\n'\n");
  (0, import_node_fs.chmodSync)(fakeCodex, 448);
  const result = (0, import_node_child_process.spawnSync)(process.execPath, [runtimeFile, "--version"], {
    encoding: "utf8",
    env: {
      ...process.env,
      ...
      CODEX_CLI_PATH: import_node_path.default.join(dir, "ccr-codex-cli-stdio-profile")
    }
  });
  import_strict.default.equal(result.status, 0, result.stderr);
  import_strict.default.equal(result.stdout, "real-codex\n");
});
(0, import_node_test.default)("Codex CLI middleware launches Windows cmd shims", { skip: process.platform !== "win32" }, () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-windows-cmd-"));
  const runtimeFile = writeRuntimeScript(dir);
  const fakeCliScript = import_node_path.default.join(dir, "fake-codex.js");
  const fakeCli = import_node_path.default.join(dir, "fake-codex.cmd");
  (0, import_node_fs.writeFileSync)(fakeCliScript, "process.stdout.write(JSON.stringify(process.argv.slice(2)));\n");
  (0, import_node_fs.writeFileSync)(fakeCli, [
    "@echo off",
  ...
  import_strict.default.equal(forwardedArgs.at(-1), "--version");
  import_strict.default.equal(forwardedArgs.includes("claude-code-router"), true);
});
(0, import_node_test.default)("Windows direct profile dispatch strips the profile command arguments", { skip: process.platform !== "win32" }, () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-windows-dispatch-"));
  const runtimeFile = writeRuntimeScript(dir);
  const fakeCliScript = import_node_path.default.join(dir, "fake-claude.js");
  const fakeCli = import_node_path.default.join(dir, "fake-claude.cmd");
  (0, import_node_fs.writeFileSync)(fakeCliScript, "process.stdout.write(JSON.stringify(process.argv.slice(2)));\n");
  (0, import_node_fs.writeFileSync)(fakeCli, [
    "@echo off",
  ...
  const result = (0, import_node_child_process.spawnSync)(process.execPath, [runtimeFile, "Claude Code", "cli", "--", "--version"], {
    encoding: "utf8",
    env: {
      ...process.env,
  ...
  import_strict.default.equal(result.status, 0, result.stderr);
  import_strict.default.deepEqual(JSON.parse(result.stdout), ["--version"]);
});
(0, import_node_test.default)("Codex app-server uses a local non-OpenAI identity without credentials", { skip: process.platform === "win32" }, () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-virtual-auth-"));
  const runtimeFile = writeRuntimeScript(dir);
  const fakeCodex = import_node_path.default.join(dir, "fake-codex");
  const codexHome = import_node_path.default.join(dir, "codex-home");
  ...
  import_strict.default.deepEqual(responses[0].result, { sawBootstrap: true });
  import_strict.default.deepEqual(responses[1].result, {
    authMethod: "amazonBedrock",
    authToken: "ccr-local-profile",
    requiresOpenaiAuth: false
  });
  import_strict.default.deepEqual(responses[2].result, {
    authMethod: "amazonBedrock",
    authToken: null,
    requiresOpenaiAuth: false
  });
  import_strict.default.deepEqual(responses[3].result, {
    account: { type: "amazonBedrock", credentialSource: "codexManaged" },
    requiresOpenaiAuth: false
  });
  import_strict.default.equal((0, import_node_fs.existsSync)(import_node_path.default.join(codexHome, "auth.json")), false);
});
(0, import_node_test.default)("Codex app-server reads but never overwrites an existing ChatGPT auth file", { skip: process.platform === "win32" }, () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-preserve-auth-"));
  const runtimeFile = writeRuntimeScript(dir);
  const fakeCodex = import_node_path.default.join(dir, "fake-codex");
  const codexHome = import_node_path.default.join(dir, "codex-home");
  const authFile = import_node_path.default.join(codexHome, "auth.json");
  ...
  (0, import_node_fs.mkdirSync)(codexHome, { recursive: true });
  (0, import_node_fs.writeFileSync)(authFile, JSON.stringify(existingAuth));
  (0, import_node_fs.writeFileSync)(fakeCodex, [
    "#!/usr/bin/env node",
    "const readline = require('node:readline');",
    "const input = readline.createInterface({ input: process.stdin, crlfDelay: Infinity });",
  ...
  import_strict.default.equal(result.status, 0, result.stderr);
  const responses = result.stdout.trim().split(/\r?\n/).map((line) => JSON.parse(line));
  import_strict.default.deepEqual(responses[0].result, {
    authMethod: "chatgpt",
    authToken: token,
    requiresOpenaiAuth: true
  });
  ...
  import_strict.default.deepEqual(JSON.parse((0, import_node_fs.readFileSync)(authFile, "utf8")), existingAuth);
});
(0, import_node_test.default)("Codex app-server bridges shared ChatGPT auth into an isolated profile without copying it", { skip: process.platform === "win32" }, () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-shared-auth-"));
  const runtimeFile = writeRuntimeScript(dir);
  const fakeCodex = import_node_path.default.join(dir, "fake-codex");
  const codexHome = import_node_path.default.join(dir, "codex-home");
  const sharedAuthFile = import_node_path.default.join(dir, "shared-auth.json");
  ...
  (0, import_node_fs.mkdirSync)(codexHome, { recursive: true });
  (0, import_node_fs.writeFileSync)(sharedAuthFile, JSON.stringify(sharedAuth));
  (0, import_node_fs.writeFileSync)(fakeCodex, [
    "#!/usr/bin/env node",
    "const readline = require('node:readline');",
    "const input = readline.createInterface({ input: process.stdin, crlfDelay: Infinity });",
  ...
  import_strict.default.equal((0, import_node_fs.existsSync)(import_node_path.default.join(codexHome, "auth.json")), false);
  import_strict.default.deepEqual(JSON.parse((0, import_node_fs.readFileSync)(sharedAuthFile, "utf8")), sharedAuth);
});
(0, import_node_test.default)("Codex app-server delegates public Git marketplaces and leaves account-private marketplaces empty", { skip: process.platform === "win32" }, () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-official-plugins-"));
  const runtimeFile = writeRuntimeScript(dir);
  const fakeCodex = import_node_path.default.join(dir, "fake-codex");
  const codexHome = import_node_path.default.join(dir, "codex-home");
  (0, import_node_fs.mkdirSync)(codexHome, { recursive: true });
  ...
  import_strict.default.equal(responses.get(1).result.marketplaces[0].name, "local");
  import_strict.default.equal(responses.get(1).result.marketplaces[0].path, "/native/local");
  import_strict.default.deepEqual(responses.get(2).result, {
    marketplaces: [],
    marketplaceLoadErrors: [],
    featuredPluginIds: []
  });
});
(0, import_node_test.default)("Codex app-server delegates the native model catalog unchanged", { skip: process.platform === "win32" }, () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-native-models-"));
  const runtimeFile = writeRuntimeScript(dir);
  const fakeCodex = import_node_path.default.join(dir, "fake-codex");
  const codexHome = import_node_path.default.join(dir, "codex-home");
  (0, import_node_fs.mkdirSync)(codexHome, { recursive: true });
  ...
  import_strict.default.equal(result.status, 0, result.stderr);
  import_strict.default.deepEqual(JSON.parse(result.stdout.trim()).result, {
    data: [{ id: "native-model", hidden: true }],
    nextCursor: null
  });
});
(0, import_node_test.default)("Claude Code wrapper leaves the scoped profile model as an environment default", { skip: process.platform === "win32" }, () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-wrapper-"));
  const runtimeFile = writeRuntimeScript(dir);
  const { fakeCli, outputFile } = writeFakeClaudeCli(dir);
  (0, import_node_child_process.execFileSync)(process.execPath, [runtimeFile, "-p", "hi"], {
    env: {
      ...process.env,
  ...
  const observed = JSON.parse((0, import_node_fs.readFileSync)(outputFile, "utf8"));
  import_strict.default.deepEqual(observed.argv, ["-p", "hi"]);
  import_strict.default.equal(observed.env.ANTHROPIC_MODEL, "Fusion/kimisearch");
  import_strict.default.equal(observed.env.CCR_CLAUDE_CODE_MODEL, "Fusion/kimisearch");
});
(0, import_node_test.default)("Claude Code wrapper preserves an explicit model argument", { skip: process.platform === "win32" }, () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-wrapper-"));
  const runtimeFile = writeRuntimeScript(dir);
  const { fakeCli, outputFile } = writeFakeClaudeCli(dir);
  (0, import_node_child_process.execFileSync)(process.execPath, [runtimeFile, "--model", "Provider/manual", "-p", "hi"], {
    env: {
      ...process.env,
      ANTHROPIC_MODEL: "Fusion/kimisearch",
      CCR_CLAUDE_CODE_MODEL: "Fusion/kimisearch",
  ...
  const observed = JSON.parse((0, import_node_fs.readFileSync)(outputFile, "utf8"));
  import_strict.default.deepEqual(observed.argv, ["--model", "Provider/manual", "-p", "hi"]);
});
(0, import_node_test.default)("Claude Code wrapper injects the ToolHub MCP config into real CLI args", { skip: process.platform === "win32" }, () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-wrapper-"));
  const runtimeFile = writeRuntimeScript(dir);
  const { fakeCli, outputFile } = writeFakeClaudeCli(dir);
  const mcpConfigFile = import_node_path.default.join(dir, "toolhub-mcp.json");
  (0, import_node_child_process.execFileSync)(process.execPath, [runtimeFile, "-p", "hi"], {
    env: {
      ...process.env,
  ...
  const observed = JSON.parse((0, import_node_fs.readFileSync)(outputFile, "utf8"));
  import_strict.default.deepEqual(observed.argv, ["--mcp-config", mcpConfigFile, "-p", "hi"]);
  import_strict.default.equal(observed.env.CCR_CLAUDE_CODE_MCP_CONFIG, mcpConfigFile);
});
(0, import_node_test.default)("Claude Code wrapper does not duplicate an explicit MCP config argument", { skip: process.platform === "win32" }, () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-wrapper-"));
  const runtimeFile = writeRuntimeScript(dir);
  const { fakeCli, outputFile } = writeFakeClaudeCli(dir);
  const envMcpConfigFile = import_node_path.default.join(dir, "toolhub-mcp.json");
  const explicitMcpConfigFile = import_node_path.default.join(dir, "manual-mcp.json");
  (0, import_node_child_process.execFileSync)(process.execPath, [runtimeFile, "--mcp-config", explicitMcpConfigFile, "-p", "hi"], {
    env: {
      ...process.env,
  ...
  const observed = JSON.parse((0, import_node_fs.readFileSync)(outputFile, "utf8"));
  import_strict.default.deepEqual(observed.argv, ["--mcp-config", explicitMcpConfigFile, "-p", "hi"]);
});
(0, import_node_test.default)("OpenCode bot worker keeps commands responsive while preserving per-conversation turn order", { skip: process.platform === "win32" }, async () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-opencode-bot-"));
  const runtimeFile = writeRuntimeScript(dir);
  const fakeOpenCode = import_node_path.default.join(dir, "fake-opencode");
  const fakeSdk = import_node_path.default.join(dir, "fake-bot-gateway-sdk.mjs");
  const callsFile = import_node_path.default.join(dir, "opencode-calls.jsonl");
  const repliesFile = import_node_path.default.join(dir, "bot-replies.jsonl");
  const stateDir = import_node_path.default.join(dir, "bot-state");
  const otherProject = import_node_path.default.join(dir, "other-project");
  const configFile = import_node_path.default.join(dir, "opencode.json");
  (0, import_node_fs.mkdirSync)(stateDir, { recursive: true });
  (0, import_node_fs.mkdirSync)(otherProject, { recursive: true });
  (0, import_node_fs.writeFileSync)(import_node_path.default.join(stateDir, "opencode-bot-sessions.json"), JSON.stringify({
    version: 1,
    conversations: {
  ...
  (0, import_node_fs.writeFileSync)(configFile, "{}\n");
  (0, import_node_fs.writeFileSync)(fakeOpenCode, [
    "#!/usr/bin/env node",
    "const fs = require('node:fs');",
  ...
  (0, import_node_fs.chmodSync)(fakeOpenCode, 448);
  (0, import_node_fs.writeFileSync)(fakeSdk, [
  ...
  let stderr = "";
  const child = (0, import_node_child_process.spawn)(process.execPath, [runtimeFile, "opencode-bot-worker", "--workspace-name", "OpenCode Test"], {
    env: {
      ...process.env,
  ...
  try {
    const replies = await waitForJsonLines(repliesFile, 10, 7e3, () => stderr);
    const calls = await waitForJsonLines(callsFile, 5, 2e3, () => stderr);
    const replyTexts = replies.map((reply) => reply.intent.text);
    import_strict.default.ok(replyTexts.includes("Unknown Bot command. Send /project or /session to see available commands."));
    import_strict.default.ok(replyTexts.some((text) => /^CCR App project commands \(OpenCode\):/.test(text)));
    import_strict.default.ok(replyTexts.some((text) => /^OpenCode projects:/.test(text)));
    import_strict.default.ok(replyTexts.some((text) => /^CCR App session commands \(OpenCode\):/.test(text)));
    import_strict.default.ok(replyTexts.some((text) => /^OpenCode sessions in /.test(text)));
    import_strict.default.ok(replyTexts.some((text) => text.includes(otherProject)));
    import_strict.default.ok(!replyTexts.some((text) => text.includes("Other session")));
    import_strict.default.deepEqual(replyTexts.filter((text) => text.startsWith("reply:")), ["reply:first", "reply:help", "reply:second"]);
    const runCalls = calls.filter((call) => call.argv[0] === "run");
    import_strict.default.deepEqual(runCalls[0].argv.slice(0, 7), ["run", "--format", "json", "--dir", dir, "--title", "Bot: OpenCode Test"]);
    import_strict.default.ok(!runCalls[0].argv.includes("--auto"));
    import_strict.default.equal(runCalls[0].argv.at(-1), "first");
    import_strict.default.deepEqual(runCalls[1].argv.slice(0, 7), ["run", "--format", "json", "--dir", dir, "--session", "ses_bot_1"]);
    import_strict.default.equal(runCalls[1].argv.at(-1), "help");
    import_strict.default.deepEqual(runCalls[2].argv.slice(0, 7), ["run", "--format", "json", "--dir", dir, "--session", "ses_bot_1"]);
    import_strict.default.equal(runCalls[2].argv.at(-1), "second");
    import_strict.default.equal((0, import_node_fs.realpathSync)(runCalls[0].cwd), (0, import_node_fs.realpathSync)(dir));
    import_strict.default.equal(runCalls[0].pwd, dir);
    import_strict.default.equal(runCalls[0].config, configFile);
    import_strict.default.equal(runCalls[0].configContent, '{"provider":{}}');
    import_strict.default.equal(runCalls[0].client, "cli");
    import_strict.default.equal(runCalls[0].workerMarker, "");
    const store = JSON.parse((0, import_node_fs.readFileSync)(import_node_path.default.join(stateDir, "opencode-bot-sessions.json"), "utf8"));
    import_strict.default.equal(store.version, 3);
    import_strict.default.equal(Object.values(store.conversations)[0].sessionId, "ses_bot_1");
    import_strict.default.equal(Object.values(store.conversations)[0].projectDirectory, dir);
  } finally {
    if (child.exitCode === null && child.signalCode === null) child.kill("SIGTERM");
    await waitForChildExit(child, 3e3);
  }
});
(0, import_node_test.default)("OpenCode bot worker streams without duplicate final text replies or implicit auto approval", { skip: process.platform === "win32" }, async () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-opencode-bot-stream-"));
  const runtimeFile = writeRuntimeScript(dir);
  const fakeOpenCode = import_node_path.default.join(dir, "fake-opencode");
  const fakeSdk = import_node_path.default.join(dir, "fake-bot-gateway-sdk.mjs");
  const callsFile = import_node_path.default.join(dir, "opencode-calls.jsonl");
  ...
  try {
    const replies = await waitForJsonLines(repliesFile, 2, 7e3, () => stderr);
    const calls = await waitForJsonLines(callsFile, 1, 2e3, () => stderr);
    import_strict.default.equal(calls[0].argv.includes("--auto"), false);
    import_strict.default.equal(replies.every((reply) => reply.intent.type === "stream_text"), true);
    import_strict.default.equal(replies.some((reply) => reply.intent.type === "text" && reply.intent.text === "reply:stream me"), false);
    import_strict.default.equal(replies.filter((reply) => reply.intent.final === true).length, 1);
  } finally {
    if (child.exitCode === null && child.signalCode === null) child.kill("SIGTERM");
    await waitForChildExit(child, 3e3);
  }
});
(0, import_node_test.default)("Codex App bot worker uses native projects and sessions without enabling shell tools", { skip: process.platform === "win32" }, async () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-codex-bot-"));
  const runtimeFile = writeRuntimeScript(dir);
  const fakeCodex = import_node_path.default.join(dir, "fake-codex");
  const fakeSdk = import_node_path.default.join(dir, "fake-bot-gateway-sdk.mjs");
  const callsFile = import_node_path.default.join(dir, "codex-calls.jsonl");
  const repliesFile = import_node_path.default.join(dir, "bot-replies.jsonl");
  const stateDir = import_node_path.default.join(dir, "bot-state");
  const codexHome = import_node_path.default.join(dir, "codex-home");
  const sessionsDir = import_node_path.default.join(codexHome, "sessions", "2026", "07", "14");
  (0, import_node_fs.mkdirSync)(stateDir, { recursive: true });
  (0, import_node_fs.mkdirSync)(sessionsDir, { recursive: true });
  (0, import_node_fs.writeFileSync)(import_node_path.default.join(sessionsDir, "rollout-ses_existing.jsonl"), [
  ...
  let stderr = "";
  const child = (0, import_node_child_process.spawn)(process.execPath, [runtimeFile, "codex-bot-worker", "--workspace-name", "Codex Test"], {
    env: {
      ...process.env,
      CODEX_HOME: codexHome,
  ...
  try {
    const replies = await waitForJsonLines(repliesFile, 7, 7e3, () => stderr);
    const calls = await waitForJsonLines(callsFile, 2, 2e3, () => stderr);
    const replyTexts = replies.map((reply) => reply.intent.text);
    import_strict.default.ok(replyTexts.some((text) => text.startsWith("Codex projects:")));
    import_strict.default.ok(replyTexts.some((text) => text.startsWith("Selected project")));
    import_strict.default.ok(replyTexts.some((text) => text.startsWith("Codex sessions in") && text.includes("Existing Codex session")));
    import_strict.default.ok(replyTexts.some((text) => text.startsWith("Selected session ses_exis")));
    const agentReplies = replyTexts.filter((text) => text.startsWith("reply:"));
    import_strict.default.equal(agentReplies.length, 2);
    import_strict.default.ok(agentReplies[0].endsWith("first"));
    import_strict.default.ok(agentReplies[1].endsWith("second"));
    for (const call of calls) {
      import_strict.default.equal(call.argv[0], "exec");
      import_strict.default.ok(call.argv.includes("resume"));
      import_strict.default.ok(call.argv.includes('sandbox_mode="read-only"'));
      import_strict.default.equal((0, import_node_fs.realpathSync)(call.cwd), (0, import_node_fs.realpathSync)(dir));
    }
    const store = JSON.parse((0, import_node_fs.readFileSync)(import_node_path.default.join(stateDir, "codex-bot-sessions.json"), "utf8"));
    import_strict.default.equal(Object.values(store.conversations)[0].sessionId, "ses_codex");
  } finally {
    if (child.exitCode === null && child.signalCode === null) child.kill("SIGTERM");
    await waitForChildExit(child, 3e3);
  }
});
(0, import_node_test.default)("Codex App bot worker passes image attachments to exec and avoids duplicate stream replies", { skip: process.platform === "win32" }, async () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-codex-bot-image-"));
  const runtimeFile = writeRuntimeScript(dir);
  const fakeCodex = import_node_path.default.join(dir, "fake-codex");
  const fakeSdk = import_node_path.default.join(dir, "fake-bot-gateway-sdk.mjs");
  const callsFile = import_node_path.default.join(dir, "codex-calls.jsonl");
  ...
  try {
    ...
    import_strict.default.notEqual(imageFlagIndex, -1);
    import_strict.default.match(calls[0].argv[imageFlagIndex + 1], /screenshot\.png$/);
    import_strict.default.equal(replies.every((reply) => reply.intent.type === "stream_text"), true);
    import_strict.default.equal(replies.some((reply) => reply.intent.type === "text" && reply.intent.text === "reply:describe image"), false);
    import_strict.default.equal(replies.filter((reply) => reply.intent.final === true).length, 1);
  } finally {
    if (child.exitCode === null && child.signalCode === null) child.kill("SIGTERM");
    await waitForChildExit(child, 3e3);
...
(0, import_node_test.default)("Claude App bot worker keeps project and session selection as separate levels", { skip: process.platform === "win32" }, async () => {
  const dir = (0, import_node_fs.mkdtempSync)(import_node_path.default.join(import_node_os.default.tmpdir(), "ccr-runtime-claude-bot-projects-"));
  const runtimeFile = writeRuntimeScript(dir);
  const fakeSdk = import_node_path.default.join(dir, "fake-bot-gateway-sdk.mjs");
  const repliesFile = import_node_path.default.join(dir, "bot-replies.jsonl");
  const stateDir = import_node_path.default.join(dir, "bot-state");
  const userDataDir = import_node_path.default.join(dir, "claude-user-data");
  const sessionsDir = import_node_path.default.join(userDataDir, "local-agent-mode-sessions", "account", "organization");
  const projectA = import_node_path.default.join(dir, "project-a");
  const projectB = import_node_path.default.join(dir, "project-b");
  (0, import_node_fs.mkdirSync)(stateDir, { recursive: true });
  (0, import_node_fs.mkdirSync)(sessionsDir, { recursive: true });
  (0, import_node_fs.mkdirSync)(projectA, { recursive: true });
  (0, import_node_fs.mkdirSync)(projectB, { recursive: true });
  const writeSession = (id, title, cwd, lastActivityAt) => {
    (0, import_node_fs.writeFileSync)(import_node_path.default.join(sessionsDir, `${id}.json`), JSON.stringify({
      sessionId: id,
      cliSessionId: `cli-${id}`,
      cwd,
      userSelectedFolders: [cwd],
  ...
  let stderr = "";
  const child = (0, import_node_child_process.spawn)(process.execPath, [runtimeFile, "claude-bot-worker", "--workspace-name", "Claude Test"], {
    env: {
      ...process.env,
  ...
  try {
    const replies = await waitForJsonLines(repliesFile, 8, 7e3, () => stderr);
    const replyTexts = replies.map((reply) => reply.intent.text);
    import_strict.default.ok(replyTexts.some((text) => text.startsWith("Claude App projects:") && text.includes(projectA) && text.includes(projectB)));
    import_strict.default.ok(replyTexts.some((text) => text.startsWith("Selected project project-b")));
    import_strict.default.ok(replyTexts.some((text) => text.startsWith("Claude App sessions in project-b:") && text.includes("Session B")));
    import_strict.default.ok(!replyTexts.some((text) => text.startsWith("Claude App sessions in project-b:") && text.includes("Session A")));
    import_strict.default.ok(replyTexts.some((text) => text.startsWith("Selected session local_b: Session B")));
    import_strict.default.ok(replyTexts.some((text) => text.startsWith("Current Claude App session:") && text.includes("Session B")));
    import_strict.default.ok(replyTexts.some((text) => text.startsWith("No selected Claude App session in project project-b.")));
    import_strict.default.ok(replyTexts.includes("Unknown Bot command. Send /project or /session to see available commands."));
    const store = JSON.parse((0, import_node_fs.readFileSync)(import_node_path.default.join(stateDir, "claude-bot-sessions.json"), "utf8"));
    const entry = Object.values(store.conversations)[0];
    import_strict.default.equal(store.version, 3);
    import_strict.default.equal((0, import_node_fs.realpathSync)(entry.projectDirectory), (0, import_node_fs.realpathSync)(projectB));
    import_strict.default.equal(entry.sessionId, void 0);
  } finally {
    if (child.exitCode === null && child.signalCode === null) child.kill("SIGTERM");
    await waitForChildExit(child, 3e3);
  }
...
function writeRuntimeScript(dir) {
  const file = import_node_path.default.join(dir, "ccr-codex-cli-middleware.js");
  (0, import_node_fs.writeFileSync)(file, codexCliMiddlewareRuntimeScript());
  (0, import_node_fs.chmodSync)(file, 448);
  return file;
}
function writeFakeClaudeCli(dir) {
  const fakeCli = import_node_path.default.join(dir, "fake-claude");
  const outputFile = import_node_path.default.join(dir, "fake-claude-output.json");
  (0, import_node_fs.writeFileSync)(fakeCli, [
    "#!/usr/bin/env node",
    "const fs = require('node:fs');",
  ...
  (0, import_node_fs.chmodSync)(fakeCli, 448);
...
async function waitForJsonLines(file, count, timeoutMs, diagnostic) {
  ...
  while (Date.now() < deadline) {
    if ((0, import_node_fs.existsSync)(file)) {
      const lines = (0, import_node_fs.readFileSync)(file, "utf8").trim().split(/\r?\n/).filter(Boolean);
