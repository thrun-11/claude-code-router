        ...
        {visibleModules.has("header") ? (
          <div className="mb-2 flex min-w-0 items-start justify-between gap-2 rounded-[8px] border border-white/10 bg-white/[.04] px-2.5 py-2">
            <div className="min-w-0">
              <h1 className="truncate text-[13px] font-bold text-slate-50">{selectedProvider ? formatProviderName(selectedProvider) : "Usage Overview"}</h1>
              <p className="mt-0.5 truncate text-[10px] font-medium text-slate-400">{formatUpdated(activeStats.generatedAt)}</p>
            </div>
            <div className="shrink-0 rounded-md border border-white/10 bg-slate-900/70 px-2 py-0.5 text-[10px] font-semibold text-slate-200">30d</div>
          </div>
        ) : null}
...
function TrayDetailApp({ provider }: { provider?: string }) {
  ...
  return (
    ...
    >
      ...
      <UsageDetailPanel activeStats={snapshots[range]} accountSnapshots={accountSnapshots} modules={new Set(trayWindowModules)} provider={provider} range={range} onRangeChange={setRange} />
...
function UsageDetailPanel({
  ...
  return (
    <>
      ...
      <div className="mb-2 flex min-w-0 items-start justify-between gap-2 rounded-[8px] border border-white/10 bg-white/[.04] px-2.5 py-2">
        <div className="min-w-0">
          <h2 className="truncate text-[13px] font-bold text-slate-50">Usage Detail</h2>
          <p className="mt-0.5 truncate text-[10px] font-medium text-slate-400">{rangeLabel(range)} - {provider ? formatProviderName(provider) : "All providers"}</p>
        </div>
      ...
      {modules.has("account") ? <AccountSummaryPanel snapshots={accountSnapshots} /> : null}

      <div className="space-y-2">
        {modules.has("token-flow") ? (
        <ChartShell meta={`${formatCompactNumber(totals.requestCount)} requests`} title="Token Flow">
          <AnimatedUsageChart chartId="detail-flow" series={activeStats.series} />
        </ChartShell>
        ) : null}
  ...
  return (
    <div className="mb-3 flex min-w-0 items-center justify-between gap-3 border-b border-white/10 pb-2">
      <div className="flex min-w-0 items-center gap-2">
        ...
        <div className="min-w-0">
          <div className="truncate text-[12px] font-semibold text-slate-50">{formatCompactNumber(totalTokens)} tokens</div>
...
function AccountSummaryPanel({ snapshots }: { snapshots: ProviderAccountSnapshot[] }) {
  ...
  if (!snapshot) {
    return (
      <div className="rounded-[8px] border border-white/10 bg-white/[.03] px-3 py-2 text-[11px] font-medium text-slate-400">
        No account data configured
      </div>
    );
  }

  const meter = primaryAccountMeter(snapshot);
  const progress = meter ? meterProgress(meter) : undefined;

  return (
    <div className="rounded-[8px] border border-white/10 bg-white/[.04] p-2">
      <div className="mb-2 flex min-w-0 items-center justify-between gap-2">
        <h3 className="truncate text-[11px] font-bold text-slate-100">Account</h3>
      ...
      {meter ? (
        <>
          <div className="flex min-w-0 items-end justify-between gap-2">
            <div className="min-w-0 truncate text-[10px] font-medium text-slate-400">{meter.label}</div>
            <div className="shrink-0 text-[13px] font-bold text-slate-50">{formatAccountMeterValue(meter)}</div>
          </div>
          {progress !== undefined ? (
            <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-white/10">
              <div className={`h-full rounded-full ${accountProgressClass(snapshot.status)}`} style={{ width: `${progress}%` }} />
            </div>
          ) : null}
        </>
      ) : (
        <div className="truncate text-[10px] font-medium text-slate-400">{snapshot.message || snapshot.errors?.[0]?.message || "Unavailable"}</div>
      )}
    </div>
  );
}
...
function AnimatedUsageChart({
  ...
  return (
    <div className="min-w-0">
      <svg className="mt-2 h-16 w-full overflow-visible" preserveAspectRatio="none" role="img" viewBox="0 0 260 72" aria-label="Usage chart">
        <defs>
        ...
        <path className="tray-line-draw" d={tokenGeometry.linePath} fill="none" filter={`url(#${chartId}-glow)`} stroke="rgba(45,212,191,.95)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" vectorEffect="non-scaling-stroke" />
        <path className="tray-line-draw" d={cacheGeometry.linePath} fill="none" stroke="rgba(167,139,250,.72)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" vectorEffect="non-scaling-stroke" />
      </svg>
    </div>
  );
}
...
function TokenMixPanel({ totals }: { totals: UsageTotals }) {
  const rows = [
    { className: "bg-blue-400", label: "Input", value: totals.inputTokens },
    { className: "bg-amber-300", label: "Output", value: totals.outputTokens },
    { className: "bg-rose-300", label: "Cache", value: totals.cacheTokens }
  ];
  const max = Math.max(...rows.map((row) => row.value), 1);

  return (
    <div className="min-w-0 rounded-[8px] border border-white/10 bg-white/[.04] p-2">
      <div className="mb-2 truncate text-[11px] font-bold text-slate-100">Token Mix</div>
...
function RingMetrics({ totals }: { totals: UsageTotals }) {
  return (
    <div className="min-w-0 rounded-[8px] border border-white/10 bg-white/[.04] p-2">
      <div className="mb-2 truncate text-[11px] font-bold text-slate-100">Circular metrics</div>
...
function RingMetric({ label, value }: { label: string; value: number }) {
  ...
  return (
    <div className="relative aspect-square min-w-0 text-center">
      <svg className="h-full w-full overflow-visible" viewBox="0 0 40 40" role="img" aria-label={`${label} ${formatPercent(clamped)}`}>
        ...
        <circle
          ...
          strokeDashoffset={2 * Math.PI * 15 * (1 - clamped)}
          strokeLinecap="round"
          strokeWidth="4"
          transform="rotate(-90 20 20)"
        />
      </svg>
      <div className="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-slate-100">{formatPercent(clamped)}</div>
    </div>
  );
}
...
function ModelShareChart({ rows }: { rows: UsageComparisonRow[] }) {
  ...
  return (
    <div className="mb-2 min-w-0 rounded-[8px] border border-white/10 bg-white/[.04] p-2">
      <div className="mb-2 truncate text-[11px] font-bold text-slate-100">Model Share</div>
      <div className="space-y-1.5">
...
function primaryAccountMeter(snapshot: ProviderAccountSnapshot): ProviderAccountMeter | undefined {
  return [...snapshot.meters].sort((a, b) => {
    const aRatio = meterRemainingRatio(a) ?? 1;
    const bRatio = meterRemainingRatio(b) ?? 1;
...
function formatAccountMeterValue(meter: ProviderAccountMeter): string {
