...
function upsertRuntimeProviderList(value, provider) {
  ...
  const providers = Array.isArray(value)
    ? value.filter((candidate) => !isRecord(candidate) || stringValue(candidate.id) !== providerId)
    : [];
...
function builtinAgentConsoleAspCapabilities(options = {}) {
  ...
  return {
    ...
    protocolVersions: ["1"],
    resumeSession: true,
    runtimeAdapter: stringValue(options.runtimeAdapter) || undefined,
    sessionHistory: true,
    sessions: true,
    subagents: {
      description: stringValue(options.subagentDescription) || undefined,
    ...
    toolEvents: true,
    transports: ["stdio"],
    wireProtocol: stringValue(options.wireProtocol) || undefined
  };
}
