...
function filterUnsupportedAgentProviderConfigs(value) {
  return Array.isArray(value) ? value.filter((provider) => !isUnsupportedAgentProviderConfig(provider)) : value;
}
function filterUnsupportedSubagentConfigs(value) {
  return Array.isArray(value) ? value.filter((subagent) => !isUnsupportedAgentSubagentConfig(subagent)) : value;
}
