...
function gatewayFetchPreloadScript(): string {
  return [
    ...
    "if ((up || hasTimeout) && um) {",
    "  const undiciModule = require(um);",
    "  const { Agent, ProxyAgent } = undiciModule;",
    ...
    // Node's built-in fetch and the bundled undici can come from different
    // major versions (Node >= 25 ships undici 8 while CCR bundles undici 7).
    // A dispatcher created by one major fails inside the other's fetch with
    // UND_ERR_INVALID_ARG ("invalid onError method"), so every call that
    // carries or receives a bundled dispatcher must run through the bundled
    // undici's own fetch when the majors disagree.
    "  const fetchForDispatchers = (() => {",
    "    try {",
    "      const undiciPackage = require(um.replace(/[^\\\\/]*$/, 'package.json'));",
    "      const majorOf = (version) => parseInt(String(version).split('.')[0], 10) || 0;",
    "      const runtimeMajor = majorOf(process.versions.undici);",
    "      const bundledMajor = majorOf(undiciPackage.version);",
    "      if (runtimeMajor && bundledMajor && runtimeMajor === bundledMajor) {",
    "        return realFetch;",
    "      }",
    "    } catch {}",
    ...
    "    if (init && init.dispatcher) return fetchForDispatchers(input, init);",
    "    const dispatcher = dispatcherFor(input);",
    "    if (!dispatcher) return realFetch(input, init);",
    "    return fetchForDispatchers(input, Object.assign({}, init, { dispatcher }));",
    "  };",
