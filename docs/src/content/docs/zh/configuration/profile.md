...
| 选项 | 适用范围 | 说明 |
| --- | --- | --- |
| Agent | 全部 | 选择 Claude Code、Codex、Grok CLI 或 ZCode。Grok CLI 只支持 CLI，ZCode 只支持 App。 |
...
| 入口模式 | Claude Code、Codex、Grok CLI | `CLI & APP` 同时显示 CLI 和 App 打开入口；`CLI only` 只生成 CLI 命令；`App only` 只显示 App 打开入口。Grok CLI 固定为 `CLI only`。 |
