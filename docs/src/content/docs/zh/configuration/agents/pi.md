...
pageTitle: Pi
eyebrow: 详细配置
lead: "把 Pi 接入 CCR。Pi 仅支持 CLI，且始终限定为从 CCR 打开的会话。"
---

## 适用场景

Pi 是一款通过命令行指定供应商与模型的编码 Agent。在 CCR 中它 **仅支持 CLI**，且始终使用 **仅从 CCR 打开时生效**。

当你想让 Pi 走任意 CCR 供应商或 Fusion 模型，或运行独立的 Pi 会话时，使用本页。
...
| 字段 | 如何配置 | 效果 |
| --- | --- | --- |
| Agent | 选择 **Pi** | 在 CCR 中创建 Pi 启动入口。 |
| 配置名称 | 自由文本，例如 `Pi - Work` | 标识该配置。桌面端命令使用 `ccr-app "<名称>"`，CLI 命令使用 `ccr "<名称>"`。 |
...
- **找不到 `pi`**：确认 Pi 已安装，并且启动 CCR Desktop 的同一 shell 环境可以找到它。
