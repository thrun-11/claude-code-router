...
pageTitle: 接入供应商
eyebrow: 快速开始
lead: "页面顶部的交互式面板可直接连到你正在运行的 CCR 添加供应商；或按下方说明在桌面端配置——选择预设或自定义端点、填写凭据，CCR 会自动探测协议与模型，再用连通性检查确认。"
---
...
填写 API 地址和密钥后，CCR 会自动探测该端点支持的协议与可用模型。预设供应商默认隐藏 API 地址输入，需要时可在 **高级设置** 中覆盖。
...
团队或高频调用场景，可在凭据步骤切换到 **凭据池**，添加多条上游 Key 并设置优先级、权重和限额，CCR 会按规则在 Key 之间轮换。
